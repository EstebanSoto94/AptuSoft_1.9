...
// Type: Aptusoft.frmUnidadMedidas
...
namespace Aptusoft
{
  public class frmUnidadMedidas : Form
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.txtNombre.Leave += new EventHandler(this.txtNombre_Leave);
      ...
      this.dgvDatos.Columns.AddRange((DataGridViewColumn) this.CodigoUnidad, (DataGridViewColumn) this.DescUnidad);
      this.dgvDatos.Location = new Point(12, 73);
      ...
      this.dgvDatos.Size = new Size(351, 338);
      this.dgvDatos.TabIndex = 2;
      this.dgvDatos.ColumnHeaderMouseClick += new DataGridViewCellMouseEventHandler(this.dgvDatos_ColumnHeaderMouseClick);
      this.dgvDatos.DoubleClick += new EventHandler(this.dgvDatos_DoubleClick);
      this.CodigoUnidad.DataPropertyName = "CodigoUnidad";
      ...
      this.Name = "frmUnidadMedidas";
      this.ShowIcon = false;
      this.Text = "Modulo Unidad de Medidas";
      this.Load += new EventHandler(this.frmUnidadMedidas_Load);
      this.FormClosing += new FormClosingEventHandler(this.frmUnidadMedidas_FormClosing);
    ...
    private void cargaPermisos()
    {
      ...
      {
        if (usuarioVo.Modulo.Equals("UNIDAD MEDIDA"))
        {
    ...
    {
      this.dgvDatos.DataSource = (object) this.unidad.listaUnidadMedida().Tables[0];
      this.numFila();
    ...
    private void guardaUnidad()
    {
      ...
      try
      {
        if (MessageBox.Show("Nueva Unidad de Medida, ¿Desea Guardarla?", "Alerta", MessageBoxButtons.YesNo) != DialogResult.Yes)
          return;
        this.unidad.agregaUnidad(this.txtNombre.Text.Trim());
        this.iniciaFormulario();
        int num = (int) MessageBox.Show("Unidad de Meidad Agregada");
      }
    ...
    {
      if (this.unidad.buscaUnidadDesc(this.txtNombre.Text.Trim()) && this.btnGuardar.Enabled)
      {
        int num = (int) MessageBox.Show("Esta Unidad de Medida Ya Existe!!", "Advertencia");
        this.txtNombre.Focus();
        this.btnGuardar.Enabled = false;
        if (this._elimina)
    ...
    {
      frmMenuPrincipal._modUnidMedidas = 0;
      this.Close();
      this.Dispose();
    }
    ...
    public void buscaFila()
    {
      ...
      for (int index = 0; index < this.dgvDatos.Rows.Count; ++index)
      {
        ...
        {
          this._idUnidad = Convert.ToInt32(this.dgvDatos.Rows[index].Cells[0].Value.ToString());
    ...
    private void txtNombre_KeyPress(object sender, KeyPressEventArgs e)
    {
      if ((int) e.KeyChar != 13)
        return;
      e.Handled = false;
      if (this.unidad.buscaUnidadDesc(this.txtNombre.Text.Trim()))
      {
        this.buscaFila();
        int num = (int) MessageBox.Show("Esta Unidad de Medida Ya Existe!!", "Advertencia");
        this.txtNombre.Focus();
    ...
    private void btnModificar_Click(object sender, EventArgs e)
    {
      ...
      if (this.txtNombre.Text.Length <= 0)
        return;
      this.unidad.actualizaUnidad(new UnidadMedidaVO()
      {
        IdUnidMed = this._idUnidad,
        NombreUnidad = this.txtNombre.Text.Trim()
      });
      int num = (int) MessageBox.Show("Unidad de Medida Actualizada");
      this.iniciaFormulario();
    }

    private void btnEliminar_Click(object sender, EventArgs e)
    {
      if (MessageBox.Show("Esta seguro de Eliminar la Unidad de Medida", "Alerta", MessageBoxButtons.YesNo) != DialogResult.Yes)
        return;
      int num1 = this.unidad.hayProductosEnUnidad(this._idUnidad);
      if (num1 == 0)
      {
        this.unidad.eliminaUnidad(this._idUnidad);
        int num2 = (int) MessageBox.Show("Unidad de Medida Eliminada");
        this.iniciaFormulario();
      }
      else
      {
        int num2 = (int) MessageBox.Show("Existen [ " + num1.ToString() + " ] Articulos Asociados a Esta Unidad de Medida, No se puede Eliminar");
        this.iniciaFormulario();
      }
    }

    public void buscaUnidadDGV()
    {
      int rowIndex = this.dgvDatos.CurrentCell.RowIndex;
      this._idUnidad = Convert.ToInt32(this.dgvDatos.Rows[rowIndex].Cells[0].Value.ToString());
    ...
    private void frmUnidadMedidas_FormClosing(object sender, FormClosingEventArgs e)
    {
      frmMenuPrincipal._modUnidMedidas = 0;
    }
  }
